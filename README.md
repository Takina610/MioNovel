...
bun run verify     # 跑验收（38 项 txt + 45 项 epub + 640 份演示模式文件 + 28 项快捷键 + 102 项办公外壳断言）
bun run verify:decoy   # 只跑演示模式：括号配对、空块、重名方法、行数对齐等
bun run verify:hotkey  # 只跑快捷键：组合解析、键位匹配、录键校验
bun run verify:apps    # 只跑办公外壳：主题 token、块切分、消息/行/幻灯片、云文档首页、界面文案
...
方便和译文对照），Word 是灰底上一张 A4 纸（带标尺和页边距标记），
Excel 是网格（行号 + A 列正文 + 字数 + 类型，编辑栏跟着当前行走），
PPT 是幻灯片浏览（一张张贴着排，左侧缩略图、下方备注）。

几件所有外壳都守着的规矩：

- **会响的按钮都是真能用的。** Word 的功能区里，复制、字体、字号、行距、缩进、对齐、
  导航窗格、标尺、阅读视图、全屏、摸鱼模式按下去屏幕真的会变——因为它们恰好都是
  这个阅读器本来就有的设置。Excel 的「冻结首行」、PPT 的「备注」、企业微信功能栏上的
  通讯录 / 日程 / 微盘也都是真的（换的是左边那一栏列什么）。
- **灰着的按钮是真灰的。** 只读文档里本来就该灰的那些命令（粘贴、格式刷、字体颜色、
  加粗、查找替换、评论……）一律 `disabled`，不装成能用。飞书首页上那三张卡片的「模板库」、
...
- **不编造数据。** 会话列表里的时间是这本书真实的最近阅读时间，头像字是书里作者的首字，
  ...
  假时间戳。
- **图怎么放，每个形态不一样。** Word 页面里图正常显示、PPT 里它在一张幻灯片上；
  **飞书文档、编辑器（VS Code）、企业微信和 Excel 都不显示图**——那里写的是一行
  `![](./OEBPS/Images/pic.png)`，路径是书里的原图地址。
