...
主题列表里另有十二套是「让屏幕上是那个软件」的：**飞书 亮 / 暗**、**企业微信 亮 / 暗**、
**Word 亮 / 暗**、**Excel 亮 / 暗**、**PPT 亮 / 暗**、**1688 亮 / 暗**。
和 VS Code 那两套同一个路子——选中之后整个应用变成那副样子，不是只换配色。
...
| Excel | 开始屏幕（问候 + 新建空白工作簿 + 最近） | 一个 `.xlsx` | 一张工作表 | 底部的工作表标签 |
| PPT | 开始屏幕（问候 + 八张模板 + 最近） | 一份 `.pptx` | 一节 | 左侧的幻灯片缩略图栏 |
| 1688 | 客户工作台（功能栏 + 会话列表 + 右边的店铺概览） | 一个买家会话 | 一条消息 | 右侧「订单追踪」那一栏（一行一章） |
...
PPT 是编辑视图（一次一张：左边一栏缩略图、中间那张幻灯片（版式的两个占位框里放的是
这一张真的字）、底下一条备注带；视图页签里另有一个「幻灯片浏览」），
1688 是客服工作台（一条条消息气泡、右边那块客户档案，**每条右侧消息底下写着「已读 / 未读」
——那是按你的阅读位置算出来的**，往下读会一条条变成已读；工具条最右那个按钮能把
底部的回复框收起来（工具条留着，消息区多出一屏），再点一下放回来）。
...
- **不编造数据。** 会话列表里的时间是这本书真实的最近阅读时间，头像字是书里作者的首字，
  ...
  假时间戳。
- **图怎么放，每个形态不一样。** 真图只出现在**普通阅读器**那一档（几套日间 / 夜间主题，
  安静看一本书时插图该在）；**编辑器（VS Code）、飞书文档、企业微信、Excel、Word 和
  PPT 的幻灯片都不显示图**——那里写的是一行 `![](./OEBPS/Images/pic.png)`，
  路径是书里的原图地址（这一行说的是「这里原本有一张图」，字一个不少）。
  这条规矩只在一处定义（`lib/blocks.ts` 的 `mediaModeFor`），`verify:apps` 直接
  断言那张表：改哪个形态都跑不掉。
- **演示模式（Alt+Q）仍然只属于 VS Code 那两套**：它生成的是源代码，在 Word 里没有意义。
  摸鱼模式（Alt+S）则六套外壳都有效——它压的是正文那一片。
- **1688 那一套里三处和截图不同，是有意的**：底部那几颗药丸（截图里是「发优惠券」
  「发起订单」这些）换成了这个阅读器真的做得出来的四件事（上一章 / 下一章 / 目录 /
  阅读设置），工具条最右端是「收起 / 展开回复框」（和企业微信那一套一样：收掉底部那个
  回复框、工具条留着，消息区多出一屏）；左上角那个记号是「1688」
  （截图里是「芝麻」）；会话行上不挂未读角标（没读完的章数在功能栏「通知」那颗红角标上）。
