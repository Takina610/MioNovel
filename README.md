...
bun run verify     # 跑验收（38 项 txt + 45 项 epub + 640 份演示模式文件 + 28 项快捷键 + 75 项办公外壳断言）
bun run verify:decoy   # 只跑演示模式：括号配对、空块、重名方法、行数对齐等
bun run verify:hotkey  # 只跑快捷键：组合解析、键位匹配、录键校验
bun run verify:apps    # 只跑办公外壳：主题 token 完整性、块切分、消息/行/幻灯片
...
（普通形态里既没有文件树也没有正文区可压）。开关会记住（`mionovel:dim`）。它和演示模式互不依赖，
可以只开一个，也可以一起开：一份暗着的代码。

### 改快捷键

演示模式（`Alt+Q`）和摸鱼模式（`Alt+S`）的键都能改：阅读设置里点一下那一行的键位，
接着按你想用的组合就行——必须带 Ctrl 或 Alt，两个功能不能绑成同一个。`Esc` 取消，
`Backspace` 恢复默认。改完菜单里的提示跟着变。
...
|---|---|---|---|---|
| 飞书 | 云文档首页（我的空间 + 最近访问） | 一篇云文档 | 文档里的大标题 | 左侧「大纲」 |
...
- **灰着的按钮是真灰的。** 只读文档里本来就该灰的那些命令（粘贴、格式刷、字体颜色、
  加粗、查找替换、评论……）一律 `disabled`，不装成能用。
- **不编造数据。** 会话列表里的时间是这本书真实的最近阅读时间，头像字是书里作者的首字，
  未读角标是还没读的章数，工作表的「字数」「类型」两列是数出来的。没有假同事、假用户名、
  假时间戳。
- **图怎么放，每个形态不一样。** 飞书文档和 Word 页面里图正常显示；企业微信里它是一条
  图片消息；PPT 里它在一张幻灯片上；Excel 的单元格装不下图，所以写成一行
  `![](./OEBPS/Images/pic.png)`（和编辑器形态同一条约定）。
