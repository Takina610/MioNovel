...
头像和发信人取书里的作者名），Word 是灰底上一张 A4 纸（带标尺和页边距标记），
...
- **不编造数据。** 会话列表里的时间是这本书真实的最近阅读时间，头像字是书里作者的首字，
  ...
  假时间戳。
- **图怎么放，每个形态不一样。** Word 页面里图正常显示；企业微信里它是一条图片消息；
  PPT 里它在一张幻灯片上；**飞书文档、编辑器（VS Code）和 Excel 都不显示图**——那里写的是
  一行 `![](./OEBPS/Images/pic.png)`，路径是书里的原图地址。
...
`#8F959E` / `#F5F6F7`；企业微信是微信系那副样子（深灰功能栏、白气泡）加官网的主蓝 `#0082EF`。
配套约束与踩过的坑见 [docs/SPEC.md](docs/SPEC.md) 五 5.5 与决定记录 26–27。
