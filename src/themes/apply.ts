...
import { CHAT_TOKEN_VARS, CODE_TOKEN_VARS, DESK_TOKEN_VARS, PAGE_TOKEN_VARS, SHEET_TOKEN_VARS, SLIDE_TOKEN_VARS, TOKEN_VARS } from './vars'
...
const DESK_TOKEN_KEYS = Object.keys(DESK_TOKEN_VARS) as (keyof typeof DESK_TOKEN_VARS)[]
...
function themeBlock(theme: ReaderTheme): string {
  ...
  const chatDeclarations = theme.chat
    ...
    : ''
  const pageDeclarations = theme.page
    ? `\n${PAGE_TOKEN_KEYS.map(
  ...
  const slideDeclarations = theme.slide
    ...
    : ''
  const deskDeclarations = theme.desk
    ? `\n${DESK_TOKEN_KEYS.map(
        (key) => `  ${DESK_TOKEN_VARS[key]}: ${theme.desk![key]};`,
      ).join('\n')}`
    : ''
  // color-scheme 交给主题声明：滚动条、<select> 这些原生控件才会跟着明暗走，
  // 而不是靠我们逐个去画。
  return `:root[data-theme='${theme.id}'] {\n  color-scheme: ${theme.scheme};\n${declarations}${codeDeclarations}${chatDeclarations}${pageDeclarations}${sheetDeclarations}${slideDeclarations}${deskDeclarations}\n}`
}
